...
dataset_path = "images"  # Replace this with your actual path
...
def process_frames():
    global face_locations, face_encodings, process_frame
    while True:
...
while True:
    if frame is not None and not process_frame:
        ...
        for (top, right, bottom, left), face_encoding in zip(face_locations, face_encodings):
            ...
            cv2.rectangle(output_frame, (label_x, label_y - label_size[1] - 10), (label_x + label_size[0], label_y), (0, 255, 0), cv2.FILLED)
            cv2.putText(output_frame, label, (label_x, label_y - 5), cv2.FONT_HERSHEY_DUPLEX, 0.9, (255, 255, 255), 1)

        # Display the resulting frame
        cv2.imshow('Video', output_frame)

        # Allow processing of the next frame
        process_frame = True
